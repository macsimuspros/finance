import streamlit as st
from streamlit_gsheets import GSheetsConnection
import pandas as pd

# Impostazione pagina
st.set_page_config(page_title="ReactoFinance", page_icon="📊")

# Connessione al foglio
conn = st.connection("gsheets", type=GSheetsConnection)

# Funzione per leggere i dati in modo sicuro
def get_data():
    try:
        # Legge il foglio chiamato "Dati"
        return conn.read(worksheet="Dati", ttl="0")
    except Exception as e:
        st.error(f"Errore di connessione: {e}")
        return pd.DataFrame(columns=['ID', 'Data', 'Tipo', 'Conto', 'Importo', 'Nota'])

df = get_data()

st.title("🚀 ReactoFinance Dashboard")

# Visualizzazione dati
if not df.empty:
    st.subheader("I tuoi movimenti")
    st.dataframe(df, use_container_width=True, hide_index=True)
else:
    st.info("Il database è vuoto. Inserisci il primo movimento!")

# Form per inserire nuovi dati
with st.expander("➕ Aggiungi Movimento"):
    with st.form("add_form", clear_on_submit=True):
        col1, col2 = st.columns(2)
        with col1:
            tipo = st.selectbox("Tipo", ["Entrata", "Uscita"])
            importo = st.number_input("Importo (€)", min_value=0.0, step=0.01)
        with col2:
            conto = st.text_input("Conto", "Principale")
            nota = st.text_input("Nota")
        
        if st.form_submit_button("REGISTRA"):
            # Crea nuova riga
            nuova_riga = pd.DataFrame([{
                "ID": len(df) + 1,
                "Data": pd.Timestamp.now().strftime("%d/%m/%Y"),
                "Tipo": tipo,
                "Conto": conto,
                "Importo": importo,
                "Nota": nota
            }])
            
            # Aggiorna il foglio Google
            df_updated = pd.concat([df, nuova_riga], ignore_index=True)
            conn.update(worksheet="Dati", data=df_updated)
            st.success("Dato inviato al Cloud!")
            st.rerun()
